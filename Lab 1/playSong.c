...
void playNote(uint32_t note, uint32_t durationMs)
{
	 ...
	 BUZZ = 1; // Turn on your buzzer (Please Edit)
	delay_us(note/2);
	 BUZZ = 0; // Turn off your buzzer (Please Edit)
	delay_us(note/2);
	 t += note;
	 }
	 }
...
void button_task(void *pvParameters)
{
	...
	while(1){
		button = GPIO_ReadInputDataBit(BUTTON_PORT, BUTTON_PIN); //stm32f4xx_gpio.c
		if(!button){
			playSong(song);
		}
